...
static int wait_command_completion(u32 rt,u32 expected_type,u8*slot_out,u8*cc_out){
    (void)rt;
...
__attribute__((ms_abi)) u64 efi_main(void*image_handle,void*system_table){
    ...
    serial_init();
    line("QEVARYNOX-UEFI-XHCI-DIRECT-STAGE2-V1");
    line("MODE=DESTRUCTIVE_QEMU_XHCI_REINIT");
    line("STATE=START");
    ...
    zero(dcbaa,sizeof(dcbaa));zero(command_ring,sizeof(command_ring));zero(event_ring,sizeof(event_ring));zero(erst,sizeof(erst));
    ...
    u8 slot=0,cc=0;
    if(!wait_command_completion(rtsoff,TRB_TYPE_COMMAND_COMPLETION,&slot,&cc)){
    ...
    line("XHCI_DIRECT_TAKEOVER_STAGE2=PASS");
    line("XHCI_ADDRESS_DEVICE=NOT_ATTEMPTED");
