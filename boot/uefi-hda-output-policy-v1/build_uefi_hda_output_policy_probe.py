...
MARKS={
 'start': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nSTATE=START\r\nMODE=AUTO_OUTPUT_POLICY\r\nTRANSPORT=IMMEDIATE_COMMAND\r\nEND\r\n',
 'hda': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nHDA_CONTROLLER_CODEC=PASS\r\nEND\r\n',
 'vendor': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nGET_PARAMETER_VENDOR_ID=PASS\r\nVENDOR_AGNOSTIC=PASS\r\nEND\r\n',
 'nodes': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nROOT_SUBORDINATE_NODES=PASS\r\nEND\r\n',
 'afg': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nAUDIO_FUNCTION_GROUP=PASS\r\nAFG_SUBORDINATE_WIDGETS=PASS\r\nEND\r\n',
 'widgets': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nAUTO_DAC_WIDGET=PASS\r\nAUTO_OUTPUT_PIN_WIDGET=PASS\r\nPIN_OUTPUT_CAP=PASS\r\nEND\r\n',
 'route': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nPIN_CONNECTION_LIST=PASS\r\nAUTO_PIN_TO_DAC_DIRECT_ROUTE=PASS\r\nNO_FIXED_WIDGET_NIDS=PASS\r\nEND\r\n',
 'pin_default': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nPIN_CONFIG_DEFAULT=PASS\r\nOUTPUT_DEVICE_CLASS=ANALOG_OUTPUT\r\nEND\r\n',
 'amp': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nDAC_OUTPUT_AMP_CAP=PASS\r\nDAC_OUTPUT_AMP_PROGRAM=PASS\r\nDAC_OUTPUT_AMP_VERIFY=PASS\r\nEND\r\n',
 'done': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nSTATUS=PASS\r\nEND\r\n',
 'no_hda': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nSTATUS=BLOCKED\r\nREASON=HDA_PCI_NOT_FOUND\r\nEND\r\n',
 'bad_hda': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nSTATUS=BLOCKED\r\nREASON=HDA_CONTROLLER_OR_CODEC_FAILED\r\nEND\r\n',
 'verb_timeout': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nSTATUS=BLOCKED\r\nREASON=IMMEDIATE_COMMAND_TIMEOUT\r\nEND\r\n',
 'vendor_fail': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nSTATUS=BLOCKED\r\nREASON=CODEC_VENDOR_INVALID\r\nEND\r\n',
 'nodes_fail': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nSTATUS=BLOCKED\r\nREASON=SUBORDINATE_NODES_INVALID\r\nEND\r\n',
 'afg_fail': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nSTATUS=BLOCKED\r\nREASON=AUDIO_FUNCTION_GROUP_NOT_FOUND\r\nEND\r\n',
 'widget_fail': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nSTATUS=BLOCKED\r\nREASON=AUTO_OUTPUT_WIDGETS_NOT_FOUND\r\nEND\r\n',
 'route_fail': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nSTATUS=BLOCKED\r\nREASON=DIRECT_OUTPUT_ROUTE_NOT_FOUND\r\nEND\r\n',
 'pin_default_fail': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nSTATUS=BLOCKED\r\nREASON=OUTPUT_PIN_DEFAULT_UNSUPPORTED\r\nEND\r\n',
 'amp_fail': b'QEVARYNOX-UEFI-HDA-OUTPUT-POLICY-V1\r\nSTATUS=BLOCKED\r\nREASON=OUTPUT_AMP_PROGRAM_VERIFY_FAILED\r\nEND\r\n',
}
...
def build():
 ...
 c=Code()
 c.emit(b'\x53\x41\x54\x41\x55\x41\x56\x41\x57\x56\x57')
 ...
 c.emit(b'\x89\xc1\x83\xe1\x06\x83\xf9\x04'); c.rel32(b'\x0f\x85','bar32')
 c.emit(b'\x44\x89\xe8\x83\xc8\x14'); c.rel32(b'\xe8','pci_read32')
 c.emit(b'\x48\xc1\xe0\x20\x44\x89\xf1\x48\x83\xe1\xf0\x48\x09\xc8\x48\x89\xc3')
 c.rel32(b'\xe9','bar_ready')
 c.label('bar32')
 c.emit(b'\x44\x89\xf0\x25\xf0\xff\xff\xff\x48\x89\xc3')
 c.label('bar_ready')
 c.emit(b'\x48\x85\xdb'); c.rel32(b'\x0f\x84','fail_bad_hda')
 ...
 c.emit(b'\x3d\xff\xff\xff\xff'); c.rel32(b'\x0f\x84','fail_vendor')
 c.emit(b'\x85\xc0'); c.rel32(b'\x0f\x84','fail_vendor')
 serial('vendor')
 ...
 # Query the discovered AFG's subordinate widget range (parameter 04h).
 ...
 # r14d = first dynamically discovered Audio Output converter NID.
 # edi   = first dynamically discovered output-capable Pin Complex NID.
 c.emit(b'\x45\x31\xf6\x31\xff')
 c.label('widget_scan')
 ...
 c.emit(b'\x45\x85\xf6'); c.rel32(b'\x0f\x85','widget_next')
 c.emit(b'\x41\x89\xf6'); c.rel32(b'\xe9','widget_next')
 c.label('maybe_pin')
 c.emit(b'\x83\xf9\x04'); c.rel32(b'\x0f\x85','widget_next')
 c.emit(b'\x85\xff'); c.rel32(b'\x0f\x85','widget_next')

 # Require Pin Capabilities bit 4 (output capable).
 ...
 c.emit(b'\x45\x85\xf6'); c.rel32(b'\x0f\x84','fail_widget')
 c.emit(b'\x85\xff'); c.rel32(b'\x0f\x84','fail_widget')
 serial('widgets')

 # Query the selected Pin Complex connection list length dynamically.
 c.emit(b'\x44\x89\xe0\xc1\xe0\x1c\x89\xf9\xc1\xe1\x14\x09\xc8\x0d\x0e\x00\x0f\x00')
 c.rel32(b'\xe8','immediate')
 c.emit(b'\x3d\xff\xff\xff\xff'); c.rel32(b'\x0f\x84','fail_verb_timeout')
 c.emit(b'\x89\xc1\x80\xe1\x7f'); c.rel32(b'\x0f\x84','fail_route')
 c.emit(b'\xa8\x80'); c.rel32(b'\x0f\x85','route_long')

 # Short-form NID connection list: entry 0 is bits 6:0.
 c.label('route_short')
 c.emit(b'\x44\x89\xe0\xc1\xe0\x1c\x89\xf9\xc1\xe1\x14\x09\xc8\x0d\x00\x02\x0f\x00')
 c.rel32(b'\xe8','immediate')
 c.emit(b'\x3d\xff\xff\xff\xff'); c.rel32(b'\x0f\x84','fail_verb_timeout')
 c.emit(b'\x25\x7f\x00\x00\x00\x44\x39\xf0'); c.rel32(b'\x0f\x85','fail_route')
 c.rel32(b'\xe9','route_ok')

 # Long-form NID connection list: entry 0 is bits 14:0.
 c.label('route_long')
 c.emit(b'\x44\x89\xe0\xc1\xe0\x1c\x89\xf9\xc1\xe1\x14\x09\xc8\x0d\x00\x02\x0f\x00')
 c.rel32(b'\xe8','immediate')
 c.emit(b'\x3d\xff\xff\xff\xff'); c.rel32(b'\x0f\x84','fail_verb_timeout')
 c.emit(b'\x25\xff\x7f\x00\x00\x44\x39\xf0'); c.rel32(b'\x0f\x85','fail_route')

 c.label('route_ok')
 serial('route')

 # Read the discovered output pin Config Default (GET_CONFIG_DEFAULT F1Ch).
 # Accept analog Line Out (0), Speaker (1), or Headphone Out (2).
 c.emit(b'\x44\x89\xe0\xc1\xe0\x1c\x89\xf9\xc1\xe1\x14\x09\xc8\x0d\x00\x1c\x0f\x00')
 c.rel32(b'\xe8','immediate')
 c.emit(b'\x3d\xff\xff\xff\xff'); c.rel32(b'\x0f\x84','fail_verb_timeout')
 c.emit(b'\xc1\xe8\x14\x83\xe0\x0f\x83\xf8\x02'); c.rel32(b'\x0f\x87','fail_pin_default')
 serial('pin_default')

 # Query the discovered DAC Output Amplifier Capabilities (parameter 12h).
 c.emit(b'\x44\x89\xe0\xc1\xe0\x1c\x44\x89\xf1\xc1\xe1\x14\x09\xc8\x0d\x12\x00\x0f\x00')
 c.rel32(b'\xe8','immediate')
 c.emit(b'\x3d\xff\xff\xff\xff'); c.rel32(b'\x0f\x84','fail_verb_timeout')
 c.emit(b'\x85\xc0'); c.rel32(b'\x0f\x84','fail_amp')

 # SET_AMP_GAIN_MUTE: output, left+right, unmuted, gain 0x40.
 c.emit(b'\x44\x89\xe0\xc1\xe0\x1c\x44\x89\xf1\xc1\xe1\x14\x09\xc8\x0d\x40\xb0\x03\x00')
 c.rel32(b'\xe8','immediate')
 c.emit(b'\x3d\xff\xff\xff\xff'); c.rel32(b'\x0f\x84','fail_verb_timeout')

 # GET_AMP_GAIN_MUTE left output and verify exact unmuted gain 0x40.
 c.emit(b'\x44\x89\xe0\xc1\xe0\x1c\x44\x89\xf1\xc1\xe1\x14\x09\xc8\x0d\x00\xa0\x0b\x00')
 c.rel32(b'\xe8','immediate')
 c.emit(b'\x3d\xff\xff\xff\xff'); c.rel32(b'\x0f\x84','fail_verb_timeout')
 c.emit(b'\x25\xff\x00\x00\x00\x83\xf8\x40'); c.rel32(b'\x0f\x85','fail_amp')

 # GET_AMP_GAIN_MUTE right output and verify exact unmuted gain 0x40.
 c.emit(b'\x44\x89\xe0\xc1\xe0\x1c\x44\x89\xf1\xc1\xe1\x14\x09\xc8\x0d\x00\x80\x0b\x00')
 c.rel32(b'\xe8','immediate')
 c.emit(b'\x3d\xff\xff\xff\xff'); c.rel32(b'\x0f\x84','fail_verb_timeout')
 c.emit(b'\x25\xff\x00\x00\x00\x83\xf8\x40'); c.rel32(b'\x0f\x85','fail_amp')
 serial('amp'); serial('done')
 ...
 c.label('fail_route'); serial('route_fail'); c.rel32(b'\xe9','return_fail')
 c.label('fail_pin_default'); serial('pin_default_fail'); c.rel32(b'\xe9','return_fail')
 c.label('fail_amp'); serial('amp_fail')
 c.label('return_fail'); c.emit(b'\xb8\x01\x00\x00\x00')
 c.label('return'); c.emit(b'\x48\x83\xc4\x20\x5f\x5e\x41\x5f\x41\x5e\x41\x5d\x41\x5c\x5b\xc3')
...
def main():
 if len(sys.argv)!=2: raise SystemExit('usage: build_uefi_hda_output_policy_probe.py OUTPUT_EFI')
 image=build(); validate(image); p=Path(sys.argv[1]); p.parent.mkdir(parents=True,exist_ok=True); p.write_bytes(image)
 print('OS_UEFI_HDA_OUTPUT_POLICY_BUILD=PASS'); print('bytes='+str(len(image))); print('sha256='+hashlib.sha256(image).hexdigest())
