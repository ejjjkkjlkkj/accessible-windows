...
static int run_pcm_dma(void) {
    ...
    const u32 frames = 12000;
    const u32 bytes = frames * 4;
    volatile short *pcm = (volatile short *)(usize)(base + pcm_off);
    for (u32 i = 0; i < frames; ++i) {
        u32 phase = i % 72;
        short sample = phase < 36 ? 12000 : -12000;
        pcm[i * 2] = sample;
        pcm[i * 2 + 1] = sample;
    }

    volatile u64 *bdl_addr = (volatile u64 *)(usize)base;
    volatile u32 *bdl_len = (volatile u32 *)(usize)(base + 8);
    volatile u32 *bdl_flags = (volatile u32 *)(usize)(base + 12);
    *bdl_addr = base + pcm_off;
    *bdl_len = bytes;
    *bdl_flags = 1;
    fence();
    ...
    *(volatile u32 *)(sd + 0x08) = bytes;
    *(volatile u16 *)(sd + 0x0c) = 0;
...
__attribute__((ms_abi)) u64 efi_main(void *image_handle, void *system_table) {
    (void)image_handle;
    serial_init();
    marker("QEVARYNOX-UEFI-HDA-GRAPH-PLAYBACK-V1");
    marker("STATE=START");
    ...
    if (!run_pcm_dma()) {
        marker("STATUS=BLOCKED");
        marker("REASON=HDA_GRAPH_DMA_PLAYBACK_FAILED");
        return 1;
    }
    marker("HDA_GRAPH_DMA_PLAYBACK=PASS");
