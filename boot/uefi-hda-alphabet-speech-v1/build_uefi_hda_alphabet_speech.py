...
MARKS={
 ...
 'dma': b'QEVARYNOX-UEFI-HDA-ALPHABET-SPEECH-V1\r\nUNIT_BANK_COPY=PASS\r\nBDL_RUNTIME_CHARACTER_SCHEDULE=PASS\r\nEND\r\n',
 'codec': b'QEVARYNOX-UEFI-HDA-ALPHABET-SPEECH-V1\r\nCODEC_DAC_STREAM=PASS\r\nCODEC_PIN_OUTPUT=PASS\r\nEND\r\n',
 'stream': b'QEVARYNOX-UEFI-HDA-ALPHABET-SPEECH-V1\r\nOUTPUT_STREAM_DESCRIPTOR=PASS\r\nFORMAT_48K_S16_STEREO=PASS\r\nBDL_ENTRIES=RUNTIME\r\nEND\r\n',
 'progress': b'QEVARYNOX-UEFI-HDA-ALPHABET-SPEECH-V1\r\nLPIB_PROGRESS=PASS\r\nRUNTIME_ALPHABET_SELECTION_HDA=PASS\r\nEND\r\n',
 'done': b'QEVARYNOX-UEFI-HDA-ALPHABET-SPEECH-V1\r\nSTATUS=PASS\r\nEND\r\n',
 'no_hda': b'QEVARYNOX-UEFI-HDA-ALPHABET-SPEECH-V1\r\nSTATUS=BLOCKED\r\nREASON=HDA_PCI_NOT_FOUND\r\nEND\r\n',
 'bad_hda': b'QEVARYNOX-UEFI-HDA-ALPHABET-SPEECH-V1\r\nSTATUS=BLOCKED\r\nREASON=HDA_CONTROLLER_OR_CODEC_FAILED\r\nEND\r\n',
 'alloc': b'QEVARYNOX-UEFI-HDA-ALPHABET-SPEECH-V1\r\nSTATUS=BLOCKED\r\nREASON=DMA_ALLOC_FAILED\r\nEND\r\n',
 'verb': b'QEVARYNOX-UEFI-HDA-ALPHABET-SPEECH-V1\r\nSTATUS=BLOCKED\r\nREASON=CODEC_VERB_FAILED\r\nEND\r\n',
 'stream_fail': b'QEVARYNOX-UEFI-HDA-ALPHABET-SPEECH-V1\r\nSTATUS=BLOCKED\r\nREASON=STREAM_DMA_NO_PROGRESS\r\nEND\r\n',
 'key_fail': b'QEVARYNOX-UEFI-HDA-ALPHABET-SPEECH-V1\r\nSTATUS=BLOCKED\r\nREASON=ALPHABET_KEY_INVALID\r\nEND\r\n',
...
def make_pcm():
 ...
 bank_names=sorted({name for sequence in LETTER_UNITS.values() for name in sequence})
 missing=[name for name in bank_names if name not in units]
 if missing:
  raise SystemExit(f'missing native speech units: {missing!r}')
 ...
 for ch,sequence in LETTER_UNITS.items():
  ...
  if bytes(bank)==utterance:
   raise SystemExit(f'unit bank accidentally equals pre-concatenated letter {ch}')
...
def build():
 ...
 L={'maxaddr':0,'keybuf':8,'dac_nid':16,'pin_nid':20}
 ...
 # Accept both lowercase and uppercase A-Z from EFI UnicodeChar.
 for ch in LETTER_UNITS:
  c.emit(b'\x66\x3d'+struct.pack('<H',ord(ch)))
  c.rel32(b'\x0f\x84',f'select_{ch}')
  c.emit(b'\x66\x3d'+struct.pack('<H',ord(ch.upper())))
  c.rel32(b'\x0f\x84',f'select_{ch}')
 c.rel32(b'\xe9','fail_key')

 def emit_character(label,marker,sequence):
  c.label(label)
  serial(marker)
  total=0
