...
$sourceBlob=Get-ProofField 'UEFI_SOURCE_BLOB' '[0-9a-f]{40}'
if($ExpectedSourceBlob){
  if($ExpectedSourceBlob -notmatch '^[0-9a-f]{40}[ordered]@{
  STATUS='PASS'
...
if($AudibleSpeakerConfirmed){
  ...
  if($sourceBlob -ne $ExpectedSourceBlob){ throw "Physical proof source blob $sourceBlob does not match expected $ExpectedSourceBlob" }
}
